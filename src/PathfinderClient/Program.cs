...
services.AddOpenIddict()
    ...
    .AddClient(options =>
    {
        ...
        {
            Issuer = new Uri("https://localhost:7126/", UriKind.Absolute),
...
var resource = await GetFootprintsAsync(provider, token);
...
static async Task<string> GetFootprintsAsync(IServiceProvider provider, string token)
{
    using var client = provider.GetRequiredService<HttpClient>();
    using var request = new HttpRequestMessage(HttpMethod.Get, "https://localhost:7126/2/footprints");
